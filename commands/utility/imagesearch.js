...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addStringOption((option) => option.setName('searchquery').setDescription('Query to search images for.').setRequired(true))
		.setDefaultMemberPermissions(PermissionFlagsBits.SendMessages),
	async execute(client, interaction, settings) {
		const searchQuery = interaction.options.getString('searchquery');
		...
		collector.on('collect', async (int) => {
			...
			switch (int.customId) {
				case 'BACK':
					if (curPage === 0) return await interaction.deferUpdate();
					curPage--;
					await int.update({ embeds: [embeds[curPage]] });
					break;

				case 'NEXT':
					if (curPage > embeds.length - 1) return await interaction.deferUpdate();
					curPage++;
					await int.update({ embeds: [embeds[curPage]] });
					break;
