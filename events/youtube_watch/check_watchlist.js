	...
	async execute(client) {
		...
		for (const watchedChannel of youtubeWatchlist) {
			try {
				...
				if (!lastCheckedVideo || (latestVideo.id.split(':')[2] !== lastCheckedVideo.id && new Date(latestVideo.pubDate) > new Date(lastCheckedVideo.publishDate))) {
					...
					const targetChannel = targetGuild.channels.cache.get(watchedChannel.channelId) || (await targetGuild.channels.fetch(watchedChannel.channelId));
					targetChannel.name;
					if (!targetChannel) {
						await youtubeNotificationData.findOneAndDelete({
							_id: watchedChannel._id,
